...
const getOne = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!id || id === 'undefined') return res.status(400).json({ success: false, message: 'Invalid customer ID' });
    ...
    const { data: invoices } = await db.from('invoices').select('*').eq('customer_id', id).order('invoice_date', { ascending: false }).limit(10);
