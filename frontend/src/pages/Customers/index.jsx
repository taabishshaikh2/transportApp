...
import { Modal, PageHeader, SearchBar, FormGroup, Input, fmtCurrency } from '../../components/common';
import { Spinner } from '../../components/common';

const EMPTY = {
  companyName: '', contactPerson: '', phone: '', email: '',
  city: '', state: 'Maharashtra', address: '',
  gstin: '', dhlGstin: '', creditDays: 30, openingBalance: 0,
};

// Map Supabase snake_case row → camelCase form
const toForm = (c) => ({
    ...
    <>
      <div className="form-grid">
        <FormGroup label="Company Name *" full>
          <Input value={form.companyName} onChange={set('companyName')} placeholder="Reliance Industries Ltd" />
        </FormGroup>
        <FormGroup label="Contact Person">
          <Input value={form.contactPerson} onChange={set('contactPerson')} placeholder="Vikram Shah" />
        </FormGroup>
        <FormGroup label="Phone">
          <Input value={form.phone} onChange={set('phone')} placeholder="9800000000" />
        </FormGroup>
        <FormGroup label="Email">
          <Input type="email" value={form.email} onChange={set('email')} placeholder="ops@company.com" />
        </FormGroup>
        ...
        <FormGroup label="GSTIN">
          <Input value={form.gstin} onChange={set('gstin')} placeholder="27XXXXX0000X1ZX" />
        </FormGroup>
        <FormGroup label="DHL GSTIN (if applicable)">
          <Input value={form.dhlGstin} onChange={set('dhlGstin')} />
        </FormGroup>
        <FormGroup label="Credit Days">
          <Input type="number" value={form.creditDays} onChange={set('creditDays')} min={0} />
        </FormGroup>
        ...
        <FormGroup label="Address" full>
          <textarea className="form-textarea" value={form.address} onChange={set('address')} placeholder="Full address" />
        </FormGroup>
      </div>
...
const Customers = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await customerAPI.list({ search: search || undefined });
  ...
  const save = async (form) => {
    if (modal.mode === 'edit') {
      ...
      if (!id) return toast.error('Customer ID missing — please reload the page');
  ...
  const remove = async (c) => {
    if (!c?.id) return toast.error('Customer ID missing');
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th>City</th><th>GSTIN</th><th>Credit</th>
                  <th>Billed</th><th>Outstanding</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {customers.length === 0 ? (
                  ...
                  <tr key={c.id}>
                    <td><span className="badge badge-gray">{c.customer_id}</span></td>
                    <td style={{ fontWeight: 600 }}>{c.company_name}</td>
                    <td>{c.contact_person || '—'}</td>
                    <td>{c.phone || '—'}</td>
                    <td>{c.city || '—'}</td>
                    <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{c.gstin || '—'}</td>
                    <td>{c.credit_days}d</td>
                    <td style={{ fontWeight: 600 }}>{fmtCurrency(c.total_billed || 0)}</td>
